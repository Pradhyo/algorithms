...
def find_eulerian_tour(graph):
    ...
    if is_tour(nodes):
        ...
        for node1, node2 in nodes.items():
            if node1 in tour and len(node2)>0:
            # insert detour just like before
                index = tour.index(node1)
                while len(nodes[node1]) > 0 :
                    next_node = nodes[node1].pop()
                    nodes[next_node].remove(node1)
                    index+=1
                    tour.insert(index,next_node)
                    node1 = next_node
        return tour
    else:
        return None
